...
import { Text, View, StyleSheet, TouchableOpacity, Dimensions, ScrollView, Image, Alert, Animated } from 'react-native'
...
export default class DetailScreen extends Component {
    ...
    async componentDidMount() {
        ...
        this.setState({ allData: allData, backDrops: [...images], image: 'https://image.tmdb.org/t/p/original' + val.params.imageUrl })
    ...
    render() {
        const translateY = this.state.offset.interpolate({
            ...
            outputRange: [0, -Dimensions.get('window').height/1.8+70],
            extrapolate: "clamp",
          });
        return (
            <View style={styles.mainContainer}>
                <BackIcon  name={this.props.route.params.params.name} height={this.state.offset} />
                <Animated.View style={{position:'absolute',height:Dimensions.get('window').height/1.8,transform:[{translateY:translateY}]}}>
                ...
                <Animated.View    
            style={{transform:[{translateY:translateY}],flex:1,marginTop:Dimensions.get('window').height/1.8,marginBottom:-Dimensions.get('window').height/1.8}} >
                      <TabView offset={this.state.offset} />
                </Animated.View> 
...
const MainInformation = ({ uri, name, relaseDate = '', status, voteAvarage='', voteCount='' }) => {
    ...
    return (
        <View style={{ flexWrap: 'wrap', flexDirection: 'row', width: '100%', padding: 10, justifyContent: 'flex-start' }}>
            <View style={{
                padding:7,height: Dimensions.get('window').height / 7.2, translateY: -Dimensions.get('window').height / 4.2 / 2
            }}>

                <Image resizeMode='cover' source={uri ? {uri: uri} : null} style={{ borderRadius: 25, borderColor: 'white', borderWidth: 1.5, width: Dimensions.get('window').width / 3.7, height: Dimensions.get('window').height / 4.2 }} />
            </View>
...
class BackIcon extends Component {
    ...
    render(){
        ...
        const opacity =this.props.height.interpolate({
            inputRange:[0,Dimensions.get('window').height/2.1,Dimensions.get('window').height/1.8],
            outputRange:[0,0,1],
            extrapolate: 'clamp',
        })
    ...
    return (
       <Animated.View style={{
        ...
        width:'100%',
        height:70, }}>
            <FontAwesome5Icon style={{padding:7,position:'absolute',alignSelf:'center'}} color='white' name='chevron-left' size={29} />
            <Animated.View style={{
...
const BackGroundView = ({ images }) => {
    return <View style={{ height: Dimensions.get('window').height / 3.4 }}>
        ...
        <SliderBox resizeMode='cover' dotColor="white"
            ...
            }} sliderBoxHeight={Dimensions.get('window').height / 3.4} images={images} />
    </View>
}
